...
| `FAULT (timer?)` after more than 5.5 h of charging | the BQ25185's 6 h safety timer. From flat, 1S4P needs 8–9 h: the firmware restarts charging once per USB session (`safety timer ran out before full: charging restarted once`) | nothing, the first time - the restart is not published as a fault. A second timer fault in the same session stays, Matter shows `BatReplacementNeeded` and sends a `BatChargeFaultChange` event (SafetyTimeout): unplug USB-C and plug it in again. If it recurs, measure the cells - one may be faulty; [`NUTZUNG.md`](NUTZUNG.md) §3/§4 |
